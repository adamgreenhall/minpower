...
def css():
    ...
    os.system(
        "cp source/_static/default.css {bd}_static/default.css".format(bd=build_dir)
    )
...
def html():
    ...
    if os.system(
        "sphinx-build -P -b html -d {build_dir}doctrees  source {build_dir}".format(
            build_dir=build_dir
        )
    ):
