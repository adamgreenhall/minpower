...
def init_store(power_system, times, data):
    ...
    hydro_hist = pd.DataFrame()
    for gen in power_system.get_generators_hydro():
        hydro_hist = hydro_hist.append(
            gen.flow_history.join(
            Series(str(gen), name='name', index=gen.flow_history.index)
            ))
    if power_system.has_hydro:
        storage['hydro_vars'] = hydro_hist[
            ['name', 'elevation', 'outflow', 'power', 'spill']]
    else:
        storage['hydro_vars'] = hydro_hist
    return storage
