...
forecast = pd.read_csv("wind_forecast.csv", parse_dates=True, index_col=0, squeeze=True)
tz = forecast.index.tz
days = pd.date_range(forecast.index[0], forecast.index[-1], freq="D", tz=tz)[
    :-1
] - pd.DateOffset(hours=6)
...
for day in days:
    ...
    scenarios = pd.DataFrame(
        [fcst.values, fcst.values], index=[0, 1], columns=[str(t) for t in fcst.index]
    )
